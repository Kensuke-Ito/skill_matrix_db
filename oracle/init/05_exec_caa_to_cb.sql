...
FOR r IN (
  ...
  WHERE
    owner = 'CB'
) LOOP EXECUTE IMMEDIATE 'GRANT SELECT, INSERT, UPDATE, DELETE ON ' || r.owner || '.' || r.object_name || ' TO cb_to_caa_role';
...
FOR r IN (
  ...
  WHERE
    owner = 'CB'
) LOOP EXECUTE IMMEDIATE 'GRANT SELECT ON ' || r.owner || '.' || r.object_name || ' TO cb_to_caa_role';
...
) LOOP EXECUTE IMMEDIATE 'GRANT EXECUTE ON ' || r.owner || '.' || r.object_name || ' TO cb_to_caa_role';
...
GRANT
SELECT
  ON CB.CBM_USR TO CAA;

GRANT
SELECT
  ON CB.CBT_USRATTRINF TO CAA;

GRANT
SELECT
  ON CB.CBM_AUTH TO CAA;

GRANT EXECUTE ON CB.CBM_SSKMEM_TABLE TO CAA;

GRANT
SELECT
  ON CB.CBV_SECG TO CAA;

GRANT
SELECT
  ON CB.CBM_SSKKSI TO CAA;
...
GRANT
SELECT
  ON CB.CBM_USRKBN TO CAA;

GRANT
SELECT
  ON CB.CBM_LGIN TO CAA;

GRANT
SELECT
  ON CB.CBT_CSTMINF TO CAA;

GRANT
SELECT
  ON CB.CBT_PRMI TO CAA;